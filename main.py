...
def get_user_by_username():
    session = Session()
    session.query(User).filter_by(username="sam").one_or_none()
    session.close()
...
def create_posts_for_users():
    ...
    post_1 = Post(title="Tesla Model X", author=user_sam)
    post_2 = Post(title="Nissan Leaf", author=user_john)
...
def create_tags():
    ...
    tags_names = ["news", "tesla", "audi", "nissan"]
